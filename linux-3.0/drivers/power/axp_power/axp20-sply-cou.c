...
#define CONFIG_WISKY
#ifdef CONFIG_WISKY
#define CAP_CHANGLE_MIN_SECONDS   60
#define CAP_CHANGLE_MIN_TIMES  (CAP_CHANGLE_MIN_SECONDS / 10)
static int cap_count_diff	  = 0;
#endif
...
int usb_valid_flag;
EXPORT_SYMBOL_GPL(usb_valid_flag);
int axp_usbvol(void)
{
	axp_usbvolflag = 1;
	//printk("axp_usbvolflag:%d\n",axp_usbvolflag);
	return 0;
}
...
static void axp_change(struct axp_charger *charger)
{
	...
	usb_valid_flag=charger->usb_valid;//ltm add for zet6221_touch
...
static void axp_charging_monitor(struct work_struct *work)
{
...
#ifdef CONFIG_WISKY
	if (axp_debug) {
		DBG_PSY_MSG("wisky axp_charging_monitor AXP20_CAP val = 0x%x  rt_rest_vol = %d \n", val, rt_rest_vol);
	}
#endif
...
#ifdef CONFIG_WISKY
			if(cap_count2 >= CAP_CHANGLE_MIN_TIMES) {
#else
	...
	if (ABS(charger->rest_vol - rt_rest_vol) > 40) {
		if (cap_count_diff > CAP_CHANGLE_MIN_TIMES / 2) {

			if (charger->rest_vol > rt_rest_vol && charger->rest_vol > 0) {
				DBG_PSY_MSG("Correct10*: charging charger->rest_vol = %d  rt_rest_vol = %d \n", charger->rest_vol, rt_rest_vol);
				charger->rest_vol--;
				saved_cap--;
				Set_Rest_Cap(charger,saved_cap);
			}
			else if(charger->rest_vol < rt_rest_vol && charger->rest_vol < 99) {
				DBG_PSY_MSG("Correct11*: charging charger->rest_vol = %d  rt_rest_vol = %d \n", charger->rest_vol, rt_rest_vol);
				charger->rest_vol++;
				saved_cap++;
				Set_Rest_Cap(charger,saved_cap);
			}
	...
	if(axp_debug){
#ifdef CONFIG_WISKY
		DBG_PSY_MSG("wisky2 charger->rest_vol = %d rt_rest_vol = %d", charger->rest_vol, rt_rest_vol);
#endif	
...
#ifdef CONFIG_WISKY
			uint8_t	minute;
			axp_read(charger->master, 0x8A,	&minute);
			printk("wisky axp20_resume minute = %d\n", minute);
			if (minute >> 7) 
				charger->rest_vol = 100;
			else {
				charger->rest_vol = pre_rest_vol + (int)minute * 60 / CAP_CHANGLE_MIN_SECONDS;
				if (charger->rest_vol > 100)
					charger->rest_vol = 100;
			}
#else
