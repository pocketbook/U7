...
static unsigned int  CTPM_FW_LEN=0;
//{
//#include "ft_app.i"
//};
extern int input_ctp_module_flg;
static unsigned char CTPM_FW_T801_PINGBO[] __initdata =
{
#include "ft_app_t801_pingbo.i"
};
static unsigned char CTPM_FW_M858_PINGBO[] __initdata =
{
#include "ft_app_m858_pingbo.i"
};
/*
...
static int ctp_fetch_sysconfig_para(void)
{
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch_ex("ctp_para", "ctp_name3", (int *)(&name), &type, sizeof(name)/sizeof(int))){
		pr_err("%s: script_parser_fetch err. \n", __func__);
		goto script_parser_fetch_err;
	}
	...
	if (!strcmp("T801-A13", MachName))
	{
		CTPM_FW = CTPM_FW_T801_PINGBO;
		CTPM_FW_LEN = sizeof(CTPM_FW_T801_PINGBO)/sizeof(CTPM_FW_T801_PINGBO[0]);
		printk("ctp choose %s\n",MachName);
	}
	else if (!strcmp("M858-A13", MachName))
	{
		CTPM_FW = CTPM_FW_M858_PINGBO;
		CTPM_FW_LEN = sizeof(CTPM_FW_M858_PINGBO)/sizeof(CTPM_FW_M858_PINGBO[0]);
		printk("ctp choose %s\n",MachName);
	}
	else
	{
		printk("ctp mahc nothing  use defaut\n");
		CTPM_FW = CTPM_FW_T801_PINGBO;
		CTPM_FW_LEN = sizeof(CTPM_FW_T801_PINGBO)/sizeof(CTPM_FW_T801_PINGBO[0]);
		printk("ctp choose %s\n",MachName);
	}
	...
	if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp_twi_addr3", &twi_addr, sizeof(twi_addr)/sizeof(__u32))){
		pr_err("%s: script_parser_fetch err. \n", name);
		goto script_parser_fetch_err;
	}
...
#define BUFFER_LEN (2)            //len == 2 
   ...
   // cmd_write(0x07,0x00,0x00,0x00,1);
	...
	if ( 0xa6 == a ||a < b ){
		...
		i_ret =  fts_ctpm_fw_upgrade(&pbt_buf[0],CTPM_FW_LEN);
		if (i_ret != 0){
...
static bool ft5xts_i2c_test()
{
	...
	{
		ret =ft5x_i2c_txdata(test_data, 1);	//Test i2c.
		if (ret == 1)
			break;
		msleep(5);
	}
