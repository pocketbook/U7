...
//int pcf8563_get_datetime(struct i2c_client *client, struct rtc_time *tm);
#define F25_ALARM
//#define RTC_ALARM_DEBUG

static struct i2c_driver pcf8563_driver;
static __u32 twi_id = 0;
 int pcf8563_set_datetime(struct i2c_client *client, struct rtc_time *tm);
/* Addresses to scan */
static union{
 ...
 int pcf8563_set_datetime(struct i2c_client *client, struct rtc_time *tm)
{
    ...
    */
	...
	if(leap_year > 2069 || leap_year < 1970) {
		//pcf8563_get_datetime(client,tm);
		
		printk("leap_year:%d\n",leap_year);
		tm->tm_year=75;
		printk("reset tm->tm_year=110\n");
...
static int pcf8563_get_datetime(struct i2c_client *client, struct rtc_time *tm)
{
	...
	if (rtc_valid_tm(tm) < 0){
		...
		if (tm->tm_year < 70)
		printk("tm->tm_year < 70\n");
		else if (((unsigned)tm->tm_mon) >= 12)
		printk("(unsigned)tm->tm_mon) >= 12\n");	
		else if(tm->tm_mday < 1)
		printk("tm->tm_mday < 1\n");
		else if(tm->tm_mday > rtc_month_days(tm->tm_mon, tm->tm_year + 1900))
		printk("tm->tm_mday > rtc_month_days(tm->tm_mon, tm->tm_year + 1900)\n");
		else if(((unsigned)tm->tm_hour) >= 24)
		printk("((unsigned)tm->tm_hour) >= 24\n");
		else if(((unsigned)tm->tm_min) >= 60)
		printk("((unsigned)tm->tm_min) >= 60\n");
		else if(((unsigned)tm->tm_sec) >= 60)
		printk("((unsigned)tm->tm_sec) >= 60\n");	
		#endif
				...
				const struct i2c_device_id *id)
{
	...
	int hdle;
#if 0
	printk("ssd253x_ts_reset22 \n");

	hdle = gpio_request_ex("ctp_para", "ctp_reset");
	if(!hdle){
		printk("error ssd253x_ts_reset22 request ctp_para ctp_reset. \n");
		goto pcf8563_probe;
	}

	gpio_write_one_pin_value(hdle, 1, "ctp_reset");
	mdelay(10);//15

	gpio_write_one_pin_value(hdle, 0, "ctp_reset");
	mdelay(10);//15
